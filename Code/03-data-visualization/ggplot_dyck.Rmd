...
```{r scatterplots, echo=TRUE, eval=TRUE}
...
```{r ggplot shorthand, class.source = 'fold-hide', echo = FALSE}
...
```{r step 1, echo = FALSE, include = FALSE}
...
```{r step 2, echo = FALSE, include = FALSE}
...
```{r violoin plots, class.source = 'fold-hide', echo =- FALSE}
...
```{r geom_col piped, class.source = 'fold-hide'}
...
What happens?
```{r name plots, class.source = 'fold-hide', echo = FALSE}
  # Save plots ----------------------
...
ggplot(sumTC, aes(x = dose, 
  ...
  # altering specific theme elements
  theme(axis.text = element_text(size = 12),
        axis.title = element_text(size = 14), 
        plot.title = element_text(hjust = 0.5,
                                  size = 16),
  ...
  # manually setting fill colors and labels
   scale_fill_manual(values = c('#E69F00', '#999999'),
                     labels = c('Orange juice', 'Ascorbic acid')) +
...
Familiarize yourself a bit with the data if you haven't already
```{r read in data, class.source = 'fold-hide'}

stommel_data <- read_csv('Data/stommel_mastertable_dryad.csv')

# print first few rows of data
head(stommel_data)

# look at internal structure of variables
str(stommel_data)

# get summary of data
summary(stommel_data)
```

> Here is a link to data information on [dryad](https://datadryad.org/stash/dataset/doi:10.5061/dryad.4qrfj6qgx)

Time to make a plot on your own!

You're going to plot the distance from available surface water for zebras in the national park. To get practice with various gpplot functions and making more-or-less publication quality graphs, make sure your plot follows the guidelines below. 

1. Subset the data to just observations of Zebras   

2. Create a scatterplot with species count as a function of the *rounded* distance to water    

3. Make the shape **and** color determined by river class   

4. Customize the shapes and colors (with a greyscale palette)   

5. Ensure there is 1 legend with informative labels    

6. Create an informative plot title (centered) **and** axis titles, with units (if necessary) and capitlization for the first word and any proper nouns.   

7. Set the plot so it shows 0 - 7000 by 1000 on the x axis, and 0 - 20 by 5 on the y axis    

8. The plot should have only major gridlines, a border on the left and bottom of the plot, and minimum text size of 14 with increasing size for axis titles and main title.   

>*hint: we didn't cover all of these so use the help file or google to try to explore how to apply some of these changes to your plot. If you get stuck we will go over it at the end.*  

```{r, class.source = 'fold-hide'}

# create vectors of the labels for the plot
river_lables <- c('Upstream', 'Midstream', 'Downstream')
...
stommel_data %>% 
  ...
  # subset data to just zebras
  filter(species == 'Z') %>% 
  
  # pipe into ggplot to avoid saving to env
ggplot(.,
       aes(x = distance_to_water_rounded,
           y = species_count)) +
  
  # add points
  geom_point(aes(shape = river_class,
                 fill = river_class),
             alpha = 0.9,
             size = 2) +
  
  # custom colors and shapes
  scale_fill_manual(labels = river_lables,
                     values = c('gray100', 'gray60','gray5')) +
  scale_shape_manual(labels = river_lables,
                     values = c(24,
                                22,
                                21)) +
  
  # labels and title
  labs(x = 'Distance to water (meters)',
       y = 'Individuals',
       title = str_wrap('Distance from available surface water of zebras in Ruaha National Park',
                        40),
       fill = 'River classification',
       shape = 'River classification') + # have to include both for one legend 
